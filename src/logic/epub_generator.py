...
pre {
  font-family: "Courier New", monospace;
        # El procesamiento de texto se delega al procesador unificado
        from .epub_text_processor import EpubTextProcessor
        processor = EpubTextProcessor()
        processed_content = processor.process_chapter(content, patterns, use_python_rules=True)
  background: #f5f5f5;
  padding: 1em;
  border-radius: 4px;
  margin: 1em 0;
...
@media print {
  ...
  p {
    ...
    def generate_epub_file(
        self, metadata: EpubMetadata, chapters: List[ChapterData]
    ) -> bytes:
        """
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as epub_zip:
            ...
            container_xml, content_opf, toc_ncx, toc_xhtml = (
                self._generate_epub_structure(metadata, chapters)
            )
            ...
            for i, chapter in enumerate(chapters):
                ...
                html_content = self._generate_chapter_html(
                    chapter.title, chapter.content, metadata.patterns
                )
                epub_zip.writestr(f"{epub_dir}/{filename}", html_content)

        return zip_buffer.getvalue()

    def _generate_epub_structure(
        self, metadata: EpubMetadata, chapters: List[ChapterData]
    ):
...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">
  ...
  <body>
    ...
    def _generate_chapter_html(
        self, title: str, content: str, patterns: List[Dict]
    ) -> str:
        """Genera HTML completo de un capítulo.

        Si ``content`` ya es un documento XHTML completo (por ejemplo, la página
        de título generada por ``create_title_page_html``), este método debe
        devolverlo tal cual, sin volver a envolverlo ni procesarlo. De lo
        contrario, procesa el contenido del capítulo normalmente y lo envuelve
        en la estructura XHTML requerida.
        """
        # Detectar si el contenido ya es un documento XHTML completo.
        if content.lstrip().startswith("<?xml"):
            return content

        # Procesar el contenido del capítulo normalmente.
        from .epub_text_processor import EpubTextProcessor

        processor = EpubTextProcessor()
        processed_content = processor.process_chapter(
            content, patterns, use_python_rules=True
        )
